...
struct SearchParams {
  ...
  bool useBayesChooseMu; //M7 diagnostic: argmax-mu final chooser without useBayesSelection (bayes passenger on a stock PUCT tree)
  bool useBayesContrastChooser; //M8 phase 2a: contrast-space root chooser (docs/bayes-m8-integration.md R6); requires useBayesSearch
  double bayesContrastDeviationZ; //M8 2c-b: deviate from the reference arm only when the contrast posterior z clears this (0 = always)
  bool useBayesContrastSelection; //M8 2d: descend by contrast-voi (contested x D in contrast units); requires useBayesSearch, excludes useBayesSelection
